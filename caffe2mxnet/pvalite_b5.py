...
class pvalite_b5(Symbol):
    ...
    def inc3_unit_left(self, data, name, workspace=512):
        ...
        bn3 = mx.sym.BatchNorm(data=conv5_1, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn3')
        relu5_1 = mx.symbol.Activation(data=bn3, act_type='relu', name=name+'/relu5_1')
        if 'inc4' in name or 'inc5' in name:
            ...
            bn4 = mx.sym.BatchNorm(data=conv5_2, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn4')
            relu5_2 = mx.symbol.Activation(data=bn4, act_type='relu', name=name+'/relu5_2')
            ...
            bn5 = mx.sym.BatchNorm(data=conv5_3, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn5')
            relu5_3 = mx.symbol.Activation(data=bn5, act_type='relu', name=name+'/relu5_3')
        else:
    ...
    def inc3_unit_middle(self, data, name, workspace=512):
        ...
        bn6 = mx.sym.BatchNorm(data=conv3_1, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn6')
        relu3_1 = mx.symbol.Activation(data=bn6,act_type='relu', name=name+'/relu3_1')
        '''
        if name == 'inc3a':
            incstride = (1,1)
        else:
        ...
        if 'inc4' in name or 'inc5' in name:
            ...
            bn7 = mx.sym.BatchNorm(data=conv3_2, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn7')
            relu3_2 = mx.symbol.Activation(data=bn7,act_type='relu', name=name+'/relu3_2')
        else:        
    ...
    def inc3_unit_right(self, data, num_output, name, workspace=512):
        ...
        bn8 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=self.momentum, name=name + '_bn8')
        relu1 = mx.symbol.Activation(data=bn8, act_type='relu', name=name+'/relu1')
        return relu1
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        #if fp16:
        #    conv1 = mx.sym.Cast(data=conv1, dtype=np.float16)
        
        conv1 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, use_global_stats=True, name='bn0')
        ...
        conv2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, use_global_stats=True, name='bn1')
        ...
        conv3 = mx.sym.BatchNorm(data=conv3, fix_gamma=False, eps=2e-5, use_global_stats=True, name='bn2')
        ...
        '''
        rpn_cls_score_fabu, rpn_bbox_pred_fabu = self.get_rpn(concat, num_anchors)

        conv_new_1 = mx.sym.Convolution(data=concat, kernel=(1, 1), num_filter=256, name="conv_new_1")
