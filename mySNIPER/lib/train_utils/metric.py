...
class RPNAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        #print('>>>>>>> RPN Acc Metric pred {}'.format(pred))
...
class RPNLogLossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        label = label[keep_inds]
        cls = pred[keep_inds, label]
        #print('>>>> RPNLogLoss label {}'.format(label))
        cls += 1e-14
        cls_loss = -1 * np.log(cls)
        cls_loss = np.sum(cls_loss)
        self.sum_metric += cls_loss
...
class RCNNLogLossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        #print('>>>>>>>>  rcnn preds')
        #print(preds)
        #print('>>>>>>>>  self.pred.index rcnn label')
        #print(self.pred.index('rcnn_label'))
        if self.ohem or self.e2e:
            label = preds[self.pred.index('rcnn_label')]
        else:
            label = labels[self.label.index('rcnn_label')]
        ...
        #print(pred.shape[1])
        label = label.asnumpy().reshape(-1,).astype('int32')
        #print('>>>>> label.asnumpy {}'.format(label))
        # filter with keep_inds
        keep_inds = np.where(label != -1)[0]
        label = label[keep_inds]
        #print('>>>>>>  label[keep_inds] {}'.format(label))
        cls = pred[keep_inds, label]
        #print('>>>>> cls {0}'.format(cls))
        cls += 1e-14
        cls_loss = -1 * np.log(cls)
        cls_loss = np.sum(cls_loss)
        #print('>>>>> cls_loss {0}'.format(cls_loss))
