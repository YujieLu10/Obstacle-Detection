...
def main():
    ...
    context = [mx.gpu(int(4))]
    #context = [mx.gpu(int(gpu)) for gpu in config.gpus.split(',')]
    if not os.path.isdir(config.output_path):
        os.mkdir(config.output_path)
    for filename in os.listdir(args.im_path):
        # Get image dimensions
        filepath = os.path.join(args.im_path, filename)
        width, height = Image.open(filepath).size

        # Pack image info
        roidb = [{'image': filepath, 'width': width, 'height': height, 'flipped': False}]
        ...
        '''
        '''
        db_info.classes = [u'BG',u'car', u'van', u'bus', u'truck', u'forklift', u'person',
                       u'person-sitting', u'bicycle', u'motor', u'open-tricycle', u'close-tricycle', u'water-block',
                       u'cone-block', u'other-block', u'crash-block', u'triangle-block', u'warning-block', u'small-block', u'small-block', u'large-block', u'bicycle-group',
                       u'person-group', u'motor-group', u'parked-bicycle', u'parked-motor', u'cross-car']
        '''
        ...
        for s in config.TEST.SCALES:
            ...
            tester.set_scale(s)
            # Perform detection
            all_detections.append(tester.get_detections(vis=False, evaluate=False, cache_name='cache'))

        # Aggregate results from multiple scales and perform NMS
        tester = Tester(None, db_info, roidb, None, cfg=config, batch_size=1)
        file_name, out_extension = os.path.splitext(os.path.basename(filepath))
        #print('>>> all detections {}'.format(all_detections))
        all_detections = tester.aggregate(all_detections, vis=True, cache_name=None, vis_path='/data/luyujie/val0629_pva_8cls_3',
                                            vis_name='{}'.format(file_name), vis_ext=out_extension)
