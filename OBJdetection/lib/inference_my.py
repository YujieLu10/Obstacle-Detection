...
class Tester(object):
    def __init__(self, module, imdb, roidb, test_iter, cfg, rcnn_output_names=None,rpn_output_names=None,
        ...
        if not self.rcnn_output_names:
            ...
            self.rcnn_output_names = {
                'cls': 'cls_prob_reshape_output',
                'bbox': 'bbox_pred_fabu_reshape_output',
                'im_ids': 'im_ids'
            }
        ...
        #print('>>>>> self.class_names {}'.format(self.class_names))
    ...
    def detect(self, batch, scales):
        ...
        #print('>>>>>> gpu_rois')
        #print('>>>>> self.rpn_output_names {}'.format(self.rpn_output_names))
        #print('>>>>> gpu_out')
        #print('>>>>> self.rcnn_output_names {}'.format(self.rcnn_output_names))
        for i, (gpu_out, gpu_scales, gpu_shapes) in enumerate(zip(outputs, scales, im_shapes)):
            #print(gpu_out)
            gpu_rois = gpu_out[self.rpn_output_names['rois']].asnumpy()
            # Reshape crois
        ...
        for i in range(self.num_images):
            for j in range(1, self.num_classes):
                ...
                for all_cls_dets, valid_range in zip(scale_cls_dets, self.cfg.TEST.VALID_RANGES):
                    ...
                    cls_dets = cls_dets[valid_ids, 0:5] if len(valid_ids) > 0 else cls_dets[:, 0:5]
                    #print('>>>>> agg_dets shape')
                    #print(agg_dets.shape)
    ...
    def get_detections(self, cls_thresh=1e-3, cache_name= 'cache', evaluate= False, vis=False, vis_path=None,
        ...
        if vis:
            print('>>>>> vis_path  {} self.cfg.TEST.VISUALIZATION_PATH {}'.format(vis_path, self.cfg.TEST.VISUALIZATION_PATH))
        ...
        for batch in self.test_iter:
            ...
            for i, (cscores, cboxes, im_id) in enumerate(zip(scores, boxes, im_ids)):
                ...
                for j in range(1, self.num_classes):
                    ...
                    #print('>>>>>> rem_scores {}'.format(rem_scores))
                    #print(rem_scores.shape)
                    #print('>>>>>> rem_boxes')
                    #print(rem_boxes.shape)
            ...
            #if self.verbose:
            #print('>>>> show_info')
            print('Tester: {}/{}, Detection: {:.4f}s, Post Processing: {:.4}s'.format(min(data_counter, self.num_images), self.num_images, detect_time / data_counter, post_time / data_counter ))
            #self.show_info('Tester: {}/{}, Detection: {:.4f}s, Post Processing: {:.4}s'.format(min(data_counter, self.num_images), self.num_images, detect_time / data_counter, post_time / data_counter ))
        if self.thread_pool:
...
def detect_scale_worker(arguments):
    ...
    print('>>>>> sym_inst.get_symbol_rcnn')
    ...
    return tester.get_detections(vis=(vis and config.TEST.VISUALIZE_INTERMEDIATE_SCALES),
     evaluate=True, cache_name='dets_scale_{}x{}'.format(scale[0],scale[1]))
...
def imdb_detection_wrapper(sym_def, config, imdb, roidb, context, arg_params, aux_params, vis):
    ...
    if config.TEST.CONCURRENT_JOBS==1:
        ...
        for _, (nbatch, scale) in enumerate(zip(config.TEST.BATCH_IMAGES, config.TEST.SCALES)):
            ...
            print('>>>>>  parallel_args {}'.format(parallel_args))
...
def proposal_scale_worker(arguments):
    ...
    # Create Tester
    tester = Tester(mod, imdb, roidb, test_iter, cfg=config, batch_size=nbatch)
    print('>>>>>> proposal_scale_worker done!')
