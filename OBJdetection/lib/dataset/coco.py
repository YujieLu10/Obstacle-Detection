...
class coco(IMDB):
    def __init__(self, image_set, root_path, data_path, result_path=None, mask_size=-1, binary_thresh=None, load_mask=False):
        ...
        with open(self.trainsetpath) as tsf:
            for line in tsf:
                self.trainset.append(os.path.join(self.imagepath, line.strip() + '.jpg'))
        #print self.trainset[1]

        # val0629.txt
        with open(self.valsetpath) as vsf:
            for line in vsf:
                self.valset.append(os.path.join(self.imagepath, line.strip() + '.jpg'))
    ...
    def _print_detection_metrics(self, coco_eval):
        ...
        print '~~~~ Mean and per-category AR @ IoU=%.2f,%.2f] ~~~~' % (IoU_lo_thresh, IoU_hi_thresh)
